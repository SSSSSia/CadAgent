...
> **注**：项目从 12 工具精简到 3 工具（commit `ac10f22`），后新增 2 个视觉工具。几何分析功能已整合到 `execute_code` 的返回值中。视觉工具在未配置视觉模型时返回错误提示。
...
| `agent/prompts.py` | 系统提示词：`AGENT_SYSTEM_PROMPT`（Tool Calling）和 `REACT_SYSTEM_PROMPT`（XML 标签）。均含 `{context}` 占位符用于插入文档几何信息和参数表。包含 5 个工具的使用说明和视觉验证工作流。 |
...
视觉模型使用独立的 API 配置，可与主 LLM 使用不同的提供商/模型。通过 `vision_enabled()` 检查是否已配置。未配置时视觉工具返回错误提示，不影响其他功能。
...
视觉系统允许 Agent 截取 3D 视口截图进行视觉验证，或分析用户上传的参考图片。采用与主 LLM 分离的独立 API 客户端（`core/vision_client.py`），两者可使用不同模型和提供商。

**截图流程（`capture_view`）**：获取 FreeCAD 3D 视口 → `QBuffer` 转为 PNG base64 → 发送到视觉 API → 返回视觉分析结果。

**图片分析流程（`analyze_image`）**：用户通过面板📎按钮上传图片 → 复制到 `uploads/` 目录 → 在消息中插入 `[image: path]` 标记 → LLM 调用 `analyze_image` 工具 → 读取文件并转 base64 → 发送到视觉 API。
